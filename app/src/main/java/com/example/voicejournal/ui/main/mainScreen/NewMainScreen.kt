...
import com.example.voicejournal.ui.main.JournalItem
...
val voiceJournals = listOf(
    ...
    VoiceJournal(
        title = "Journal 4",
        content = "This is the content of journal 4",
        created = System.currentTimeMillis() - 259200000, // Three days ago
        fileName = "journal3.mp3",
        color = VoiceJournal.noteColors.random().toArgb()
    ),
    VoiceJournal(
        title = "Journal 5",
        content = "This is the content of journal 5",
        created = System.currentTimeMillis() - 259200000, // Three days ago
        fileName = "journal3.mp3",
        color = VoiceJournal.noteColors.random().toArgb()
    )
...
@Composable
fun NoteList(journals: List<VoiceJournal>, navController: NavController,modifier: Modifier =Modifier) {
...
fun VoiceJournalList(voiceJournals: List<VoiceJournal>) {
    ...
    LazyColumn(modifier = modifier) {
        groupedVoiceJournals.forEach { (date, journals) ->
            // Add a sticky header with the date
            /*val simpleDate = SimpleDateFormat("d/MMM/yy", Locale.getDefault())
            val journalDate = simpleDate.format(date)*/
            stickyHeader {
            ...
            //val groupedNotes = journal.groupBy { it.created }

            //  val sortedJournal = groupedVoiceJournals.toList().sortedByDescending { it.first }
            /*  groupedVoiceJournals.forEach { created, voiceJournals ->
                val simpleDate = SimpleDateFormat("d/MMM/yy", Locale.getDefault())
                val journalDate = simpleDate.format(created)
                StickyHeader(text = journalDate)
                Column {
                    notes.forEach { note ->
                        JournalItem(
                            voiceJournal = note,
            ...
            /*  sortedJournal.forEachIndexed { index, (created, notes) ->
                val simpleDate = SimpleDateFormat("d/MMM/yy", Locale.getDefault())
                val journalDate = simpleDate.format(created)
                if (index == 0 || created == notes.first().created ) {
                    StickyHeader(journalDate) // Display the date as a sticky header
                }
                Column {
                    notes.forEach { note ->
                        JournalItem(
